import {
  useAddDscussionMutation,
  useEditDscussionMutation,
...
const ErrandDiscussionForm = ({ errand }: { errand: IErrand }) => {
  ...
  const onFinish = (values: any) => {
    if (!discussionStore?.discussion) {
      saveDiscussionMutation({
        variables: { payload: { content: description, errand: errand._id } },
      })
        .then((result) => {
          message.success(`Errand Discussion added`);
          discussionStore.addDiscussion(
            result.data?.createDiscussion as IErrandDiscussion
          );
        })
        .catch((err) => {
      ...
      editDiscussionMutation({
        variables: {
          payload: {
            content: description,
            id: discussionStore?.discussion._id,
          },
        },
      })
        .then((result) => {
          message.success(`Errand Discussion edited`);
          discussionStore.updateDiscussion(
            result.data?.updateDiscussion as IErrandDiscussion
          );
        })
        .catch((err) => {
