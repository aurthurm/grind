...
const SingleEngagementView = () => {
  const errandStore = useErrandStore();
  const [getErrand, { loading: errandLoading, error: erandError, data: errandData }] = useGetErrandLazyQuery()
  ...
  const fetchErrand = () => {
    getErrand({ variables:  { id: router.query.id as string }}).then(result => {
      errandStore.setErrand(result?.data?.errand as IErrand)
    }).catch(err => {
  ...
  return (
    <>
      {errandData?.errand ? <ErrandDetail /> : '' }
    </>
  )
}

(SingleEngagementView as any).PageLayout = AdminLayout;
export default SingleEngagementView