...
const SingleTicketView = () => {
  ...
  const [
    getErrand,
    { loading: errandLoading, error: erandError, data: errandData },
  ] = useGetErrandLazyQuery();
  ...
  const fetchErrand = () => {
    getErrand({ variables: { id: router.query.id as string } })
      .then((result) => {
  ...
  return <>{errandData?.errand ? <ErrandDetail /> : ""}</>;
};
