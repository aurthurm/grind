...
const ProjectErrand = () => {
  ...
  useEffect(() => {
    errandStore.clear();
    fetchErrand()
    if(!router.query.errand) return;
  }, [router]);

  const fetchErrand = () => {
    getErrand({ variables:  { id: router.query.errand as string }}).then(result => {
...
(ProjectErrand as any).PageLayout = AdminLayout;
export default ProjectErrand